...
def convert_from_mathematica_to_sympy(mathematica_string, temperature=0, api_key=OPENAI_API_KEY, model="gpt-4o-mini", verbose=False):
    client = openai.OpenAI(api_key=api_key)

    init_prompt = r"""Convert the following Mathematica function into a valid SymPy expression. Ensure it is fully compatible with SymPy syntax.
        ...
        - Input: `"(3369 + 39674 n - 418216 n^8) / 738482"`
        - Output: `"Rational(3369, 738482) + Rational(39674, 738482) * n - Rational(418216, 738482) * n**8"`
        ...
        3. `Rational` objects should contain integers exclusively, no expressions that depend on `n`.
        """
    
    messages = [
        system_message(f"You are a model that converts Mathematica expressions to equivalent Python/SymPy expressions. {init_prompt}"),
        user_message(f"The Mathematica expression to convert is: {mathematica_string}")
        ]
    
    response = client.beta.chat.completions.parse(model=model, messages=messages, temperature=temperature, response_format=GPTSympy)
    messages.append(assistant_message(extract_content(response)))
    sympy_str = json.loads(messages[-1]['content'])['sympy']

    try:
        sp.sympify(sympy_str)
    except Exception as e:
        if verbose:
            print(e)
        correction_prompt = r"""Identify the cause of the error in this sympy code and correct it. Output only proper sympy code. Reply only with the corrected code.
        
        For example:
        - Input:
            - The error: TypeError: invalid input: (20*n - 17)*(32*n**3 - 48*n**2 + 22*n - 3)**3.
            - The sympy code to correct: 'Rational((-17 + 20*n) * (-3 + 22*n - 48*n**2 + 32*n**3)**3, 655360)'
        - Output:
            Rational(1, 655360) * (20*n - 17)*(32*n**3 - 48*n**2 + 22*n - 3)**3 
        """

        messages.append(system_message(correction_prompt))
        messages.append(user_message(f"The error: {str(e).split(':')[0]}. The sympy code to correct: {messages[-1]}."))
        response = client.beta.chat.completions.parse(model=model, messages=messages, temperature=temperature, response_format=GPTSympy)
        messages.append(assistant_message(extract_content(response)))
        sympy_str = json.loads(messages[-1]['content'])['sympy']

    return  sympy_str, messages


def identify_variable(formula, model='gpt-4o-mini', temperature=0, api_key=OPENAI_API_KEY):
    client = openai.OpenAI(api_key=api_key)

    messages = [
        system_message("You are a model that identifies the variable on which an expression depends. \
                       Do not confuse true dependencies with dummy variables."),
        user_message(f"What is the variable in: {formula}")
        ]
    response = client.beta.chat.completions.parse(model=model, messages=messages, temperature=temperature, response_format=GPTSympy)
    messages.append(assistant_message(extract_content(response)))

    return json.loads(messages[-1]['content'])['sympy'], messages


def convert_to_sympy(string, temperature=0, api_key=OPENAI_API_KEY, model='gpt-4o-mini', messages=[], verbose=False):
    client = openai.OpenAI(api_key=api_key)
    ...
    try:
        ...
        correction_messages = [
            system_message("You are a model that converts a string to a proper SymPy expression. \
                           The string you produce should be a valid SymPy (python) expression that will compile. \
                           Supplied is also the error message that `sympify(string)` produced."),
            user_message(f"Convert the string to a SymPy expression: {string}. Current error: {str(e)}")
            ]
        messages.extend(correction_messages)
        response = client.beta.chat.completions.parse(model=model, messages=messages, temperature=temperature, response_format=GPTSympy)
        messages.append(assistant_message(extract_content(response)))
        string = json.loads(messages[-1]['content'])['sympy']

    return string, messages


def identify_value(formula, model='gpt-4o-mini', temperature=0, api_key=OPENAI_API_KEY):
    client = openai.OpenAI(api_key=api_key)

    messages = [
        system_message("You are a model that identifies the value of an expression. \
                       The value should be on one side of the equation. \
                       Return the value as a proper SymPy expression."),
        user_message(f"What is the value of: {formula}")
        ]
    response = client.beta.chat.completions.parse(model=model, messages=messages, temperature=temperature, response_format=GPTSympy)
    messages.append(assistant_message(extract_content(response)))

    value = json.loads(messages[-1]['content'])['sympy']
    value = convert_to_sympy(value)[0]
