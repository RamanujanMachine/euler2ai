...
def convergence_rate(pcf: PCF, depth: int, limit=None, maxfev=10000) -> Tuple[float, float, float]:
    r"""
    ...
    Returns:
        A, B, C
    """
    return sc.optimize.curve_fit(paramfit, np.arange(1, depth + 1), 
                                 logerrors(pcf, depth, limit=limit), maxfev=maxfev)[0]


def q_red_growth_rate(pcf: PCF, depth: int, fit_from=0, maxfev=10000, verbose=False) -> Tuple[float, float, float]:
    r"""
    ...
    TEST_FACTOR = 3
    FACTORIAL_REDUCTION_TOLERANCE = 5e-2

    if fit_from > depth:
        raise ValueError('`fit_from` must be less than `depth`.')
    
    fit_depths = depths_for_fit(depth)

    # TODO: modify below accordingly.
    test = sc.optimize.curve_fit(paramfit, np.arange(fit_from + 1, depth // TEST_FACTOR + 1),
                                 q_reds(pcf, depth // TEST_FACTOR, log=True)[fit_from:], maxfev=maxfev)[0]
    if verbose:
        print(f'Small-scale fit to depth {depth // TEST_FACTOR}: {test}')
    if test[0] < FACTORIAL_REDUCTION_TOLERANCE:
        if verbose:
            print(f'Factorial reduction detected. Fitting with reduced model.')
        return sc.optimize.curve_fit(paramfit_NOA, np.arange(fit_from + 1, depth + 1),
                                     q_reds(pcf, depth, log=True)[fit_from:], maxfev=maxfev)[0]
    else:
        if verbose:
            print(f'Fitting with full model.')
        return sc.optimize.curve_fit(paramfit, np.arange(fit_from + 1, depth + 1),
                                     q_reds(pcf, depth, log=True)[fit_from:], maxfev=maxfev)[0]
...
def logerrors(pcf: PCF, depth: int, limit=None) -> Collection[float]:
    ...
    """
    if limit is None:
        limits = pcf.limit(list(range(2, 2 * depth + 2)))
        limit = limits[-1].as_float()
    else:
        limits = pcf.limit(list(range(2, depth + 2)))
    return [mm.log(abs(mm.mp.mpf(limit - limits[i].as_float())), 10) for i in range(depth)]
...
def q_reds(pcf: PCF, depths: List[int], log=False) -> Collection[float]:
    """
    Calculates the reduced denominators of the pcf convergents for the given depths.
    """
    depths = sorted(list(set(depths)))
    limits = pcf.limit(list(range(2, depths[-1] + 2)))
    depth_to_ind = {depth: i for i, depth in enumerate(list(range(1, depths[-1] + 1)))}
    if log:
        return [mm.log(q_red(limits[depth_to_ind[depth]]), 10) for depth in depths]
    else:
        return [q_red(limits[depth_to_ind[depth]]) for depth in depths]
