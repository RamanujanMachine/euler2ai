...
def test_gather_iterator():
    ids_of_interest = ['id0', 'id1', 'id2', 'id3', 'id4', 'id5', 'id6', 'id7', 'id8', 'id9', 'id10']
    ...
    for i, args_dict in enumerate(gather_iterator(ids_of_interest, dir_origin, dir_destin, **kwargs), start=kwargs['start_index']):
        ...
        assert args_dict['file_origin'] == f"{dir_origin}/0-10__id0__to__id10/{i}__id{i}.json"
        assert args_dict['file_destin'] == f"{dir_destin}/0-10__id0__to__id10/{i}__id{i}.json"
        assert args_dict['gather'] == {f'id{i}': {'file1': [{'e': 'equation', 'l': 1, 't': 'b'}]}}
        print(i)

    assert ids_passed == ['id3', 'id4', 'id5', 'id6', 'id7', 'id8']
