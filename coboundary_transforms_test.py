...
from coboundary_transforms import *
...
def test_multiply():
    ...
    assert multiply.multiplier == 2 * n * Matrix.eye(2)

    multiply = CobTransformMultiply(Matrix([[5, 6], [7, 8]]))
    assert multiply(matrix) == Matrix([[5, 6], [7, 8]]) * matrix
    assert multiply.U == Matrix.eye(2)
    assert multiply.multiplier == Matrix([[5, 6], [7, 8]])
...
def test_shift():
    ...
    assert cob_transform.multiplier == Matrix.eye(2)
...
def test_as_pcf():
    ...
    assert cob_transform.multiplier == polys[0] / polys[1] * Matrix.eye(2)
...
def test_compose():
    ...
    assert inf_comp_def.multiplier == Matrix.eye(2)
    ...
    assert mult_then_inf.multiplier == multiply.multiplier * inflate.inflater * Matrix.eye(2)


# TODO:
# test inflate, multiply
